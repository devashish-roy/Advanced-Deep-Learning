...
if __name__ == "__main__":
    ...
    for global_step in range(params.total_timesteps):
        ...
        if random.random() < epsilon:
            ...
            q_values = q_network(torch.tensor(obs, device=device, dtype=torch.float32)) # TODO: get q_values from the network you defined, what should the network receive as input?
        ...
        if global_step > params.learning_starts:
            if global_step % params.train_frequency == 0:
                ...
                actions = data.actions.view(-1)  # Reshape actions to be (batch_size,)

                # Reshape rewards and dones to be (batch_size,)
                ...
                q_values = q_network(data.observations)  # Shape: (B, A), where B is the batch size and A is the action space size
                action_indices = actions.unsqueeze(-1)  # Shape: (B, 1)
                old_val = q_values.gather(1, action_indices).squeeze(-1)  # Shape: (B,)
